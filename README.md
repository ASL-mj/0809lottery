...
这是一个本地 0809 多账号工作台。浏览器访问时先在 /login 完成管理员认证，工作台使用本地滑动 7 天会话 Cookie；每次 Cookie 会话请求都会将有效期延长到当前时间之后 7 天，连续 7 天未访问则需要重新登录。脚本和健康检查也可以显式发送 HTTP Basic Auth。账号数量不再受固定环境变量限制，可以在工作台中自助新增、编辑、停用、重新认证和删除。每个账号的操作相互独立：一个账号会话失效不会遮蔽其他账号的结果，也不会产生跨账号统计。
...
认证统一经过 SessionBroker，优先级固定为：有效已保存令牌 → 已保存父令牌的无副作用验证 → 刷新 Cookie 换取新令牌 → **用户显式重新认证**。页面打开、普通查询、失败重试、自动抽奖或服务重启都不会触发密码登录；单账号的认证恢复串行化，等待请求复用第一次的结果。管理员工作台会话是进程内的滑动 7 天会话：Cookie 认证的页面和 API 请求会同步续期服务端记录与 Cookie，Basic Auth 不创建或续期 Cookie；服务重启后进程内管理员会话仍会失效。
...
浏览器接口使用 GET /login 建立本地管理员会话；登录成功后 Cookie 初始有效 7 天，并在每次 Cookie 认证请求后滑动续期 7 天；脚本可显式发送 HTTP Basic Auth，Basic Auth 不建立 Cookie 会话。写接口还要求同源与 X-CSRF-Token（GET /login 或 GET / 下发的 Cookie 值）。
