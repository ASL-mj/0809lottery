...
func TestIndexContainsOnlyAccountControls(t *testing.T) {
	...
	for _, expected := range []string{"签到成功：", "签到失败：", "已签到", "领取成功：", "领取失败：", "抽奖成功：", "抽奖失败：", "跳过抽奖：", "今日已领取", "领取结果待确认", "核对结果", "data-checkin", "data-claim", "data-draw", "data-draw-count", "data-refresh", "data-activity", "data-purchase-draw", "data-unlock-pass", "领取中", "抽奖中", "刷新活动", "购买 1 抽 · $", "购买通行证 · $", "今日通行证已购买", "距下一档还需", "今日累计加抽", "全部档位已完成", "window.confirm", "仅北京时间当日有效", "await finishActionAndRefresh(account, 'claiming');", "await finishActionAndRefresh(account, 'drawing');", "await refreshDrawCountOnly(account);", "account.activity_result", "account.activity_error", "pass_unlocked", "purchase_pending", "purchase_unknown", "currentFocusTarget()", "restoreFocus(target)", "390px", ":focus-visible", ".account__identity", ".action--primary", ".metrics", ".metric__value", "<section id=\"accounts\" aria-label=\"账号列表\">", "role=\"status\"", "data-status-account", "额度统一以美元显示", "@media (max-width:1040px)", "@media (max-width:620px)", "系统运行日志", "自动抽奖执行结果，最新优先展示。", "id=\"runtime-logs-refresh\"", "/api/runtime-logs", "运行日志拉取失败：", "最近一次刷新失败：", "暂无运行日志。", "正在读取运行日志...", "morning: '早间 08:00–09:00'", "midday: '午间 13:00–14:00'", "evening: '晚间 18:00–19:00'", "log.account_label || log.account_id", "奖品：", "额度：$", "账号：", "窗口：", "renderRuntimeLogs()", "runtime-log__status", "runtime-state--error", "今日定时抽奖", "已执行 ${handled}/3", ".auto-draw__timeline", ".auto-draw__step--completed", "renderAutoDrawSchedule(account)", "/api/auto-draw-status", "refreshAutoDrawStatus()", "30000", "计划待生成", "执行中", "已跳过"} {
		if !strings.Contains(recorder.Body.String(), expected) {
